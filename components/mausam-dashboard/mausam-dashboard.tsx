...
export default function MausamDashboard() {
  ...
  return (
    <main className="min-h-screen bg-background pb-28 text-foreground transition-colors duration-500">
      ...
      {moreOpen && <section role="dialog" aria-modal="true" aria-labelledby="more-menu-title" className="fixed inset-x-4 bottom-24 z-50 rounded-2xl border border-border bg-card p-4 shadow-2xl sm:left-auto sm:right-6 sm:w-80"><div className="flex items-center justify-between"><h2 id="more-menu-title" className="font-semibold">More</h2><button aria-label="Close more menu" onClick={() => setMoreOpen(false)} className="rounded-lg p-2 text-muted-foreground hover:bg-muted"><X size={17} /></button></div><div className="mt-3 grid gap-1"><Link href="/user-profile-and-setting" onClick={() => setMoreOpen(false)} className="flex min-h-11 items-center gap-3 rounded-xl px-3 hover:bg-muted"><UserRound size={18} className="text-primary" /> User profile</Link><Link href="/help-center" onClick={() => setMoreOpen(false)} className="flex min-h-11 items-center gap-3 rounded-xl px-3 hover:bg-muted"><Settings2 size={18} className="text-primary" /> Application settings</Link><Link href="/alerts" onClick={() => setMoreOpen(false)} className="flex min-h-11 items-center gap-3 rounded-xl px-3 hover:bg-muted"><Bell size={18} className="text-primary" /> Alert preferences</Link><button onClick={() => { setTheme(isDark ? 'light' : 'dark'); setMoreOpen(false) }} className="flex min-h-11 items-center gap-3 rounded-xl px-3 text-left hover:bg-muted"><Moon size={18} className="text-primary" /> Switch to {isDark ? 'light' : 'dark'} mode</button></div></section>}
