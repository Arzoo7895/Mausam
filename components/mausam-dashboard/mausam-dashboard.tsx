          ...
          <div className="mx-auto flex max-w-[1440px] items-center justify-between px-4 py-4 md:px-8">
            ...
            <div className="hidden items-center gap-1 rounded-xl border border-border bg-card p-1 md:flex">
              {[
                ...
                { label: 'Insights', href: '/ai-insights', active: false },
                { label: 'Locations', href: '/map', active: false },
              ].map((item) => <Link key={item.label} href={item.href} className={`rounded-lg px-4 py-2 text-sm ${item.active ? 'bg-muted font-medium' : 'text-muted-foreground hover:text-foreground'}`}>{item.label}</Link>)}
            </div>
            <div className="flex items-center gap-2">
              <Link href="/map" aria-label="Search locations" className="rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-foreground"><Search size={18}/></Link>
              <button aria-label="Toggle dark mode" onClick={() => setTheme(dark ? 'light' : 'dark')} className="rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-foreground">{mounted && dark ? <Sun size={18}/> : <Moon size={18}/>}</button>
              <Link href="/alerts" aria-label="Notifications" className="relative rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-foreground"><Bell size={18}/><span className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-primary"/></Link>
              <Link href="/user-profile-and-setting" aria-label="Account and settings" className="ml-1 hidden h-8 w-8 items-center justify-center rounded-full bg-secondary text-xs font-semibold hover:bg-muted sm:flex">AS</Link>
            </div>
          </div>
          {menu && <div className="border-t border-border px-4 py-3 md:hidden"><div className="flex gap-2">{[{ label: 'Overview', href: '/dashboard' }, { label: 'Insights', href: '/ai-insights' }, { label: 'Locations', href: '/map' }].map(x => <Link key={x.label} href={x.href} onClick={() => setMenu(false)} className="rounded-lg bg-muted px-3 py-2 text-sm">{x.label}</Link>)}</div></div>}
        </header>
