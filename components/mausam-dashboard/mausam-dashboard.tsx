...
export default function MausamDashboard() {
  ...
  return (
    <main className="min-h-screen bg-background pb-28 text-foreground transition-colors duration-500">
      ...
      {moreOpen && <section role="dialog" aria-modal="true" aria-labelledby="more-menu-title" className="fixed inset-x-4 bottom-24 z-50 rounded-2xl border border-border bg-card p-4 shadow-2xl sm:left-auto sm:right-6 sm:w-80"><div className="flex items-center justify-between"><h2 id="more-menu-title" className="font-semibold">{t('nav.more')}</h2><button aria-label="Close more menu" onClick={() => setMoreOpen(false)} className="rounded-lg p-2 text-muted-foreground hover:bg-muted"><X size={17} /></button></div><div className="mt-3 grid gap-1"><button onClick={() => { setMoreOpen(false); if (requireAuthFor('User Profile')) window.location.href = '/user-profile-and-setting' }} className="flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-left hover:bg-muted"><UserRound size={18} className="text-primary" /> {t('nav.profile')}</button><button onClick={() => { setMoreOpen(false); window.location.href = '/help-center' }} className="flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-left hover:bg-muted"><Settings2 size={18} className="text-primary" /> {t('nav.appSettings')}</button><button onClick={() => { setMoreOpen(false); if (requireAuthFor('Alert Preferences')) window.location.href = '/alerts' }} className="flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-left hover:bg-muted"><Bell size={18} className="text-primary" /> {t('nav.alertPreferences')}</button><button onClick={() => { setTheme(isDark ? 'light' : 'dark'); setMoreOpen(false) }} className="flex min-h-11 items-center gap-3 rounded-xl px-3 text-left hover:bg-muted"><Moon size={18} className="text-primary" /> Switch to {isDark ? 'light' : 'dark'} mode</button></div></section>}
