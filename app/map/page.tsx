...
export default function Page() {
  ...
  return <main className="mausam-app">
    <header className="topbar"><Link href="/dashboard" className="brand"><span className="brand-mark"><Zap className="size-4 fill-current" /></span><span>Mausam <em>AI</em></span></Link><nav className="topnav"><Link href="/map" className="nav-item active"><MapIcon className="size-4" /> Intelligence</Link><Link href="/ai-insights" className="nav-item"><Activity className="size-4" /> Analytics</Link><Link href="/alerts" className="nav-item"><Bell className="size-4" /> Alerts <span className="alert-count">3</span></Link></nav><div className="top-actions"><Link href="/dashboard" className="icon-button mobile-menu" aria-label="Open menu"><Menu className="size-5" /></Link><button className="icon-button" onClick={() => setTheme(dark ? 'light' : 'dark')} aria-label="Toggle color theme">{mounted && dark ? <Sun className="size-4" /> : <Moon className="size-4" />}</button><Link href="/user-profile-and-setting" className="avatar" aria-label="Open profile">AK</Link></div></header>
