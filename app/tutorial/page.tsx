  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <div className="mx-auto grid max-w-7xl gap-10 px-5 pb-20 lg:grid-cols-[220px_minmax(0,680px)_240px] lg:px-8">
      ...
      <article className="flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-3 border-t border-border pt-6 sm:flex-row sm:items-center sm:justify-between"><Button variant="outline" onClick={() => setActiveStep(steps[Math.max(0, activeIndex - 1)].id)} disabled={activeIndex === 0}><ArrowLeft data-icon="inline-start" aria-hidden="true" /> Previous lesson</Button><Button onClick={() => { if (activeIndex < steps.length - 1) setActiveStep(steps[activeIndex + 1].id); else setCompleted(true) }}>{completed ? 'Lesson complete' : activeIndex === steps.length - 1 ? 'Mark complete' : 'Next lesson'} {completed ? <Check data-icon="inline-end" aria-hidden="true" /> : <ArrowRight data-icon="inline-end" aria-hidden="true" />}</Button></div>
      </article>
